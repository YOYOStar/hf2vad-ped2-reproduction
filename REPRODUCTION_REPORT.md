...
### 5.1 复现偏离声明（reviewer-facing；详见代码库 `PATCHES_AUDIT.md`）

> **唯一实质性方法学偏离 = 目标检测器替换**（Cascade-RCNN-R101 → torchvision Faster-RCNN-R50-FPN-v2，均 COCO 预训练）。其余全部改动为环境迁移（NumPy≥1.24 兼容、FlowNet2 CUDA 扩展改 sm_90/c++17、edflow 依赖 shim、多-chunk 评估修正、种子控制），**不触碰模型结构、损失、打分公式或超参**。

**① 结构上无 bug（本次代码审计新证据）**：下游 `getObjBboxes` 对检测结果只做 `np.vstack(所有类)` 后按 `conf_thr` 分数 + `min_area` 面积过滤——**原版从不按类别过滤**。因此替换检测器后"把全部 COCO 类合并成单数组"与原版"按类分组再 vstack"**行为等价**，未引入杂类混入或结构性缺陷；差异仅在于**检测器身份本身**（框提议 + 分数标定）。

**② 无害性经验证据（三数据集官方权重 eval，自建 cubes vs 作者原始包）**：

| 数据集 | 自建预处理 | 作者原始包 | Δ |
|---|---|---|---|
| Ped2 | 99.52 | 99.50 | 0.02 |
| Avenue | 90.64 | 90.63 | 0.01 |
| ShanghaiTech | 76.09 | 76.10 | 0.01 |

> 官方权重在"自建 cube"与"作者 cube"上打分**三集全部 Δ≤0.02 AUC**，证明检测器替换未损害"输入 cube → 官方模型打分"的保真度。

**③ 残留 caveat（诚实）**：两个 COCO 检测器**分数标定不同**，同一 `conf_thr` 会保留不同框集合；"影响有限"对运动主导的 **Ped2 最强（理论+经验）**，对外观/CVAE 更重要的 **Avenue/ShanghaiTech 仅经验成立**（从零 91.12 / 76.59 达/超论文）。若要消除该偏离，需在兼容后端上跑回原文 Cascade-RCNN（见 §7）。
